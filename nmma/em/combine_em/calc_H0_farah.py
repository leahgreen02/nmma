...
def combine_EM_events(original_KDE, EM_event, D_inj, H_range, D_range, transform_type=logit, downsample=True, N_draws=N_draws, num=None):
    ...
    #D = D_inj+D_inj*(.4*np.random.rand(N_draws)-.2) # proof of framework
    H = calc_H0(D, z)
...
ax0.errorbar(np.arange(1,len(H_medians)+1), H_medians, yerr=2*H_stds, linewidth=3, capsize=6, label='Combined Event Prediction')
