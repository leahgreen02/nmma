...
def combine_EM_events(original_KDE, EM_event, D_inj, H_range, D_range, transform_type=logit, downsample=True, N_draws=N_draws, num=None):
    ...
    #H_original_resam = original_KDE.resample(N_draws)
    #H_resam_original = logit(H_original_resam, H_range, inv_transform=True)

    #H_joint_kde = KDE_multiply(H_kde_transform, H_kde_original, downsample=downsample)
    #print(np.shape(H_resam_original), np.shape(H_resam))
    #print(np.shape(H_resam_original), np.shape(H_resam))
    #print(H_resam_original, H_resam)
    #combined_posterior = eng.PART_function(H_resam_original, H_resam)   
    combined_posterior = eng.PART_function(original_KDE, H)
    combined_posterior = np.asarray(combined_posterior)
    combined_posterior = np.array(combined_posterior).flatten()
...
def run_event_combination(H_range, D_range, N_draws=N_draws):
    ...
    for n in event_idxs:
        ...
        H_vals = combine_EM_events(original_KDE, event, D_injs[n], H_range = H_range, D_range = D_range, num=n)
        #original_KDE = {'luminosity_distance': D_kde_result, 'H0': H_kde}
        #original_KDE = {'H0': H_kde}
    ...
    H_vals = np.array(H_vals).flatten()
