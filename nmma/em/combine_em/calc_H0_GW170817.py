...
def combine_EM_events(original_KDE, EM_event, original_D_inj, D_inj, D_prior_kde, H_range, D_range, i_range, transform_type=logit, downsample=True, num=None):
    '''
    '''
    ...
    N_hist = len(D)
    #z = z + z*.2*(2*np.random.rand(N_hist)-1)

    D = D + D*.2*(2*np.random.rand(N_hist)-1)
    H = calc_H0(D, z)
    ...
    #D_prior_kde = scipy.stats.gaussian_kde(GW170817_samples['luminosity_distance'])
    D_prior = D_prior_kde.resample(N_hist)[0]
...
def run_event_combination(path_to_events, H_range, D_range, i_range, save_hist=False):
    ...
    original_D_inj = original_injection_parameters['luminosity_distance'] # working
    #original_D_inj = 40
    #original_D_inj = original_D_inj + original_D_inj*.2*(2*np.random.rand(10)-1)

    original_samples = EM_event_files[0]
    D = original_samples['luminosity_distance'] # working
    ...
    # D_prior = bilby.gw.prior.UniformSourceFrame(minimum=0.0, maximum=2000., name='luminosity_distance', cosmology = Planck18, latex_label='$D_L$') # working
    # D_prior = D_prior.prob(D) # working 
    D_prior_kde = scipy.stats.gaussian_kde(GW170817_samples['luminosity_distance'])
    D_prior = D_prior_kde.resample(N_hist)[0]
    ...
    for n, event in zip(event_indices[1:], EM_event_files[1:]):
        ...
        #D_inj = D_inj + D_inj*.2*(2*np.random.rand(10)-1)
        D_kde_result, i_kde, H_kde = combine_EM_events(original_KDE, event, original_D_inj, D_inj, D_prior_kde, H_range = H_range, D_range = D_range, i_range = i_range, num=n)
